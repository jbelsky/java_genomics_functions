...
public class TSS {
	...
	// Static Functions
	public static TreeMap<String, ArrayList<TSS>> read_in_tf(String filename)
			throws IOException {
		...
		while ((line = acs_input.readLine()) != null) {
			...
			acs_map.get(chr).add(
					new TSS(acs_line[0], chr, Integer.parseInt(acs_line[2]),
							Integer.parseInt(acs_line[3]), acs_line[4]
									.charAt(0)));
	...
	public static ArrayList<TSS> read_in_tf_file(String f) throws IOException {
		...
		while ((line = input.readLine()) != null) {
			...
			ars_list.add(new TSS(line_arr[0], line_arr[1], Integer
					.parseInt(line_arr[2]), Integer.parseInt(line_arr[3]),
					line_arr[4].charAt(0)));
	...
	public void write_reads_output(BufferedWriter output, double[] chr, int win)
			throws IOException {
