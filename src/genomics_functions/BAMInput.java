...
public class BAMInput {
	...
	public static ArrayList<String> get_chr_list(String bam_file_name) {
		...
		if (new File(bam_file_name + ".bai").exists()) {
			...
			sam_file = new SAMFileReader(new File(bam_file_name), new File(
					bam_file_name + ".bai"));
		...
		List<SAMSequenceRecord> chr_list = sam_file.getFileHeader()
	...
	public static int get_number_aligned_reads(String bam_file_name) {
		...
		for (int i = 0; i < chr_list.size(); i++) {
			...
			num_align += bi.getMetaData(chr_list.get(i).getSequenceIndex())
	...
	public static int get_number_aligned_reads(String bam_file_name,
		...
		for (String chr : chr_name) {
			...
			int ref_idx = sam_file.getFileHeader().getSequence(chr)
					.getSequenceIndex();
	...
	public static int get_number_aligned_reads_within_width(
			String bam_file_name, String[] chr_name, int read_width_low,
			int read_width_high) {

		System.out.println("Finding the number of reads between "
				+ read_width_low + " and " + read_width_high + "...");
		...
		for (String chr : chr_name) {
			...
			// Open the iterator
			SAMRecordIterator bam_itr = bam_file.queryOverlapping(chr, 1,
					get_chr_length(bam_file_name, chr));
			...
			while (bam_itr.hasNext()) {
				...
				// Only increment the read counter if the read's width falls
				// within the specified range
				if (read_width >= read_width_low
						&& read_width <= read_width_high) {
					num_reads++;
				}
		...
		System.out.println("Complete!\tThere are " + num_reads + " between "
				+ read_width_low + " and " + read_width_high);
	...
	public static int[] get_fragment_length_distribution(String bam_file_name,
			int read_width_high, String[] chr_dist) {

		System.out
				.println("Finding the read width distribution of reads with maximum width of "
						+ read_width_high + "...");
	...
	// Returns the chromosomal length
	public static double[] read_in_sam_file(String bam_file_name, String chr,
			int shift) {
		...
		// Note: Can also restrict the iterator to a particular region with
		// ".queryContained(chr_name, start, end)"
		...
		while (bam_itr.hasNext()) {
			...
			// Ensure that the corrected read position falls within the chr
			// region
			if (pos < 1) {
				pos = 1;
		...
		System.out.println("\tComplete!\nRead in " + num_reads + " reads in "
				+ (System.currentTimeMillis() - start_time) / (60F * 1000F)
				+ " minutes");
	...
	public static ChrStrPos[] read_in_sam_file_by_strand(String bam_file_name,
			String chr) {
	...
	public static ChrStrPos[] read_in_sam_file_by_strand_density(
			String bam_file_name, String chr, double bw) {
	...
	public static int[] read_in_paired_sam_file_coverage(String bam_file_name,
			String chr, int bp_coverage, int frag_low, int frag_high) {
	...
	// Returns the chromosomal length
	public static HashMap<Integer, ArrayList<Integer>> read_in_paired_sam_file_bed_hash(
			String bam_file_name, String chr, int frag_low, int frag_high) {
	...
	// Returns the chromosomal length
	public static BinReads[] read_in_sam_file_for_bin(String bam_file_name,
			String chr, int chr_start, int chr_end, int bin_width,
			int bin_step, int shift) {
		...
		// Set up the storage array
		BinReads[] chr_coverage = BinReads.create_bin_reads_array(chr_start,
				chr_end, bin_width, bin_step);
		...
		// Note: Can also restrict the iterator to a particular region with
		// ".queryContained(chr_name, start, end)"
		SAMRecordIterator bam_itr = bam_file.queryContained(chr, chr_start,
				chr_end);
		...
		while (bam_itr.hasNext()) {
			...
			chr_coverage[Arrays.binarySearch(chr_coverage, pos,
					BinReads.posCompare)].increment_count();
	...
	// Returns the chromosomal length
	public static BinReads[] read_in_paired_sam_file_for_bin(
			String bam_file_name, String chr, int chr_start, int chr_end,
			int bin_width, int bin_step, int frag_low, int frag_high) {
		...
		// Get the chr length
		// int chr_length =
		// bam_file_header.getSequence(chr).getSequenceLength();
	...
	// Returns the chromosomal length
	public static double[] read_in_sam_file_density(String bam_file_name,
			String chr, int chr_start, int chr_end, int shift, double bw) {
		...
		// double read_ratio = 2000000.0 /
		// get_number_aligned_reads(bam_file_name);
		double read_ratio = 1;
		...
		SAMRecordIterator bam_itr = bam_file.queryOverlapping(chr,
				Math.max(1, chr_start - 50), Math.min(chr_end + 50, chr_max));
	...
	// Returns the chromosomal length
	public static double[] read_in_paired_sam_file_density(
			String bam_file_name, String chr, double bw, int read_width_low,
			int read_width_high) {
	...
	// Returns the chromosomal length
	public static double[] read_in_sam_file_chip_exo_density(
			String bam_file_name, String chr, double bw) {
		...
		// Find the coverage value (normalized to 1000000 reads)
		// double read_ratio = (1000000.0 /
		// get_number_aligned_reads(bam_file_name)) * 2;
		...
		while (bam_itr.hasNext()) {
			...
			if (!read.getReadNegativeStrandFlag()) {
				...
				// Increment the chr coverage
				// DensityEst.add_to_chr_array(chr_coverage, read_ratio, pos,
				// bw);
				chr_coverage[pos]++;
