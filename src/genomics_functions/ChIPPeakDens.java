...
public class ChIPPeakDens implements Comparable<ChIPPeakDens>{
	...
	public ChIPPeakDens(String n, int c, int st, int e, int peak_pos, double peak_count){
		name = n;
		chr = c;
		start = st;
		end = e;
	...
	public static ArrayList<ChIPPeakDens> find_subnuc_peak(double[] coverage, double chip_threshold, 
														   int chr, String peak_name){
		...
		for(int p = 0; p < coverage.length; p++){
			...
			// If the count does not meet the chip_threshold and inPeak, end the peak
				peak_list.add(cp);
				inPeak = false;
			}
...
public static void write_output(BufferedWriter output, ArrayList<ChIPPeakDens> peak_list, int chr) throws IOException{
	...
	public static void write_output(BufferedWriter output, TF t, ArrayList<ChIPPeakDens> peak_list) throws IOException{
		
		// If the TF is on the reverse strand, invert the relative nucleosome positions
		if(t.getStrand() == '-'){
			for(int i = 0; i < peak_list.size(); i++)
