...
public class BinReads implements Comparable<BinReads> {
	...
	public static BinReads[] create_bin_reads_array(int chr_length, int bin_size, int step_size){
		...
		int pos = 1;
		for(int i = 0; i < storage.length; i++){
			storage[i] = new BinReads(pos, Math.min(pos + (bin_size - 1), chr_length));
			pos += step_size;
		}
	...
	public static BinReads[] create_bin_reads_array(int chr_start, int chr_end, int bin_size, int step_size){
		...
		int pos = chr_start;
		for(int i = 0; i < storage.length; i++){
			storage[i] = new BinReads(pos, Math.min(pos + (bin_size - 1), chr_end));
			pos += step_size;
		}
	...
	public static BinReads[] create_bin_reads_dist(int chr_start, int chr_end, int num_bins){
	...
	public int compareTo(BinReads br_compare){
		...
		// If the start of this BinReads is greater than the end of br_compare,
