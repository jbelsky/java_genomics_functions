...
public class DensityEst {
	...
	public static double[][] get_two_dim_smooth(double[][] dist_matrix, double val, double bw){
		...
		for(int r = 0; r < dist_matrix.length; r++){
			for(int c = 0; c < dist_matrix[r].length; c++){
				...
				// If there the hash doesn't contain the key, calculate the smoothed value
				if(!distance_hash.containsKey(dist))
					distance_hash.put(dist, normal_a * Math.exp(normal_b * Math.pow(dist, 2)));
